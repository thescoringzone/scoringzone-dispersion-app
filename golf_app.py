...
# --- 4. DATA ENGINE (SCORING + AVERAGES) ---
def format_score_cell(list_s):
    if not list_s: return "-"
    # Filter out rounds where score wasn't entered (e.g., 0)
    valid_rounds = [s for s in list_s if s.get('gross_score', 0) > 0]
    if not valid_rounds: return "-"
    
    total_gross = sum(s.get('gross_score', 0) for s in valid_rounds)
    total_par = sum(s.get('course_par', 72) for s in valid_rounds)
    to_par = total_gross - total_par
    avg_gross = total_gross / len(valid_rounds)
...
def calc_metrics(df_s, list_s, logic_type, param):
    ...
    if logic_type == "driving":
        ...
        if den > 0:
            ...
            num = (len(df_a[df_a['d'] <= rb]) * -1) + (len(df_a[df_a['d'] > rp]) * 1) # Birdies (-1) vs Bogeys (+1)
    elif logic_type == "abs":
        for s in list_s: 
            v = s.get(param, 0)
            if v != 0: num += v; den += 1
...
def create_ecga_pdf(title, df_master, df_shots):
    ...
    if len(headers) == 2: col_w = [138, 138] # Season view (2 columns)
    else: col_w = [40, 42, 42, 42, 42, 42] # Tournament view (6 columns)
    row_h = 7 
    ...
    if st.session_state.page == "Season Hub":
        ...
        for t in unique_ts:
            ...
            if t_stats:
                ...
                if v_stats:
                    avg_to_par = sum(s.get('gross_score',0) - s.get('course_par',72) for s in v_stats) / len(v_stats)
        ...
        if not final_shots.empty or final_stats:
            ...
            df_ui = df_m.copy(); df_ui['Category'] = df_ui.apply(lambda r: f"**{r['Category']}**" if r['AV / TOTAL'] == "" else r['Category'], axis=1)
            st.markdown("<style>.stTable th:first-child, .stTable td:first-child { width: 50% !important; text-align: left !important; }</style>", unsafe_allow_html=True)
            st.table(df_ui.set_index('Category'))
            
            pdf_bytes = create_ecga_pdf("Season Master (Filtered)", df_m, final_shots)
            st.download_button("📄 Download Season-Long 2-Page Report", data=pdf_bytes, file_name=f"Season_Master_Report.pdf", use_container_width=True)
            
            st.divider(); st.write("### Dispersion Analytics")
            c1, c2, c3 = st.columns(3)
            with c1: st.markdown("<h4 style='text-align: center;'>50-100m</h4>", unsafe_allow_html=True); st.image(create_target_image(final_shots[final_shots['Range'] == '50-100'], '50-100'))
            with c2: st.markdown("<h4 style='text-align: center;'>101-150m</h4>", unsafe_allow_html=True); st.image(create_target_image(final_shots[final_shots['Range'] == '101-150'], '101-150'))
            with c3: st.markdown("<h4 style='text-align: center;'>151-200m</h4>", unsafe_allow_html=True); st.image(create_target_image(final_shots[final_shots['Range'] == '151-200'], '151-200'))
            c4, c5 = st.columns(2)
            with c4: st.markdown("<h4 style='text-align: center;'>OTT: Driver</h4>", unsafe_allow_html=True); st.image(create_tee_image(final_shots[final_shots['Range'] == 'OTT: Driver'], 'OTT: Driver'))
            with c5: st.markdown("<h4 style='text-align: center;'>OTT: Others</h4>", unsafe_allow_html=True); st.image(create_tee_image(final_shots[final_shots['Range'] == 'OTT: Others'], 'OTT: Others'))
        else:
        ...
        if st.session_state.workflow_step == "Score & Driving":
            ...
            for i, r_label in enumerate(["OTT: Driver", "OTT: Others"]):
                with t_tabs[i]:
                    ...
                    if val:
                        ...
                        st.toast("☁️ Saved securely to cloud", icon="✅"); st.session_state.shots_data = load_shots(st.session_state.current_user); st.rerun()
                    if not df_v.empty:
                        st.success(f"**Tournament Sheet Stat:** {format_cell('driving', len(df_v[df_v['X'].abs() <= 10]), len(df_v), len(df_v[df_v['X'].abs() > 20]))}")
                        if st.button(f"Undo Last", key=f"un_{r_label}"): supabase.table("shots").delete().eq("id", int(df_v.iloc[-1]['id'])).execute(); st.session_state.shots_data = load_shots(st.session_state.current_user); st.rerun()
            ...
            for i, r_label in enumerate(["50-100", "101-150", "151-200"]):
                with t_tabs[i]:
                    ...
                    if val:
                        ...
                        st.toast("☁️ Saved securely to cloud", icon="✅"); st.session_state.shots_data = load_shots(st.session_state.current_user); st.rerun()
                    if not df_v.empty:
                        df_v = df_v.copy(); df_v['d'] = np.sqrt(df_v['X']**2 + df_v['Y']**2); rb, rp = get_radii(r_label)
                        st.info(f"**Tournament Sheet Stat:** {format_cell('approach', (len(df_v[df_v['d'] > rp]) - len(df_v[df_v['d'] <= rb])), len(df_v), 0)}")
                        if st.button(f"Undo Last", key=f"un_{r_label}"): supabase.table("shots").delete().eq("id", int(df_v.iloc[-1]['id'])).execute(); st.session_state.shots_data = load_shots(st.session_state.current_user); st.rerun()
            ...
            with col2:
                if sg_tot > 0:
                    st.write(f"**< 6:** {(current_stats.get('sg_inside_6',0)/sg_tot)*100:.0f}%"); st.write(f"**< 3:** {(current_stats.get('sg_inside_3',0)/sg_tot)*100:.0f}%")
                    st.write(f"**U&D:** {(current_stats.get('sg_ud',0)/sg_tot)*100:.0f}%"); st.write(f"**SGZ:** {current_stats.get('sgz_score',0)}({sg_tot})")

        elif st.session_state.workflow_step == "Putting":
            metric_cols = st.columns(2); m_putts = metric_cols[0].empty(); m_sg = metric_cols[1].empty()
            raw_grid = current_stats.get('putting_holes')
            df_putts = pd.DataFrame(raw_grid) if (raw_grid and len(raw_grid)==18) else pd.DataFrame({"Hole":[f"Hole {i}" for i in range(1,19)], "Distance (ft)":[0]*18, "Putts":[0]*18})
            ...
            m_putts.metric("Total Putts", total_putts); m_sg.metric("Total SG Putting", f"{total_sg:+.2f}")
            ...
            if lt > 0: st.write(f"**Lag:** {(ls/lt)*100:.0f}%")
            ...
            df_m = build_master_dataframe(all_ts, all_rs, mode="tournament"); df_ui = df_m.copy()
            df_ui['Category'] = df_ui.apply(lambda r: f"**{r['Category']}**" if r.get('Round 1', '') == "" else r['Category'], axis=1)
            st.markdown("<style>.stTable th:first-child, .stTable td:first-child { width: 15% !important; text-align: left !important; }</style>", unsafe_allow_html=True)
            st.table(df_ui.set_index('Category'))
            pdf_bytes = create_ecga_pdf(st.session_state.active_t, df_m, all_ts)
            st.download_button(label="📄 Download Tournament Report", data=pdf_bytes, file_name=f"{st.session_state.active_t}_Report.pdf", use_container_width=True)
